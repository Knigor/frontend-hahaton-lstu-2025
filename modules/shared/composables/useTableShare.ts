  ...
  const translationMap = {
	...
	function downloadPlan(planObject, fileName = 'План без названия'){
		...
		${content}
		  ...
		  if (navigator.msSaveBlob) { // Для IE
	...
	function createValueDisplay(value, depth) {
		...
		if (value === undefined) return '<span class="empty-value">не указано</span>';
		
		if (typeof value === 'object') {
			if (Array.isArray(value)) {
				if (value.length === 0) return '<span class="empty-value">Пустой массив</span>';
				...
				if (Object.keys(value).length === 0) return '<span class="empty-value">Пустой объект</span>';
				return createTable([value], depth);
			}
		}
